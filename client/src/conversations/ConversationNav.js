...
// Tooltip shared by every rail button.
const Tooltip = ({ children }) => (
    <span
        role="tooltip"
        className="absolute left-14 px-2 py-1 rounded-md whitespace-nowrap z-50
                   bg-surface-muted border border-surface-border text-chat-secondary text-xs
...
const ConversationNav = ({ activeTab = "chats", onTabChange }) => {
    return (
        ...
        >
            ...
            {/* ── Nav Icons ────────────────────────────────────────── */}
            <ul className="list-none m-0 p-0 flex flex-col items-center gap-1 flex-1">
                {NAV_ITEMS.map(({ key, icon: Icon, label }) => {
                    const isActive = activeTab === key;
                    // Only "Chats" has a destination today; the rest are
                    // inert until onTabChange is wired up.
                    const isAvailable = Boolean(onTabChange) || isActive;

                    return (
                        <li key={key}>
                            <button
                                type="button"
                                aria-label={isAvailable ? label : `${label} (coming soon)`}
                                ...
                                className={`
                                    ...
                                    ${isActive
                                        ...
                                        : "text-chat-faint hover:bg-surface-raised hover:text-chat-secondary"
                                    }
                                `}
                            >
                                <Icon size={20} />
                                ...
                                {isActive && (
                                    <span
                                        ...
                                        className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-0.5
                                                   w-1 h-5 bg-brand rounded-r-full"
                                    />
                                )}
                                <Tooltip>{isAvailable ? label : `${label} — coming soon`}</Tooltip>
                            </button>
                        </li>
                    );
                })}
